...
exports.run = function(bot, message_content, type, feature) {
    if (config.debug === true) bot.reply('Merge config::'+feature);

    console.log(type + '\n' + feature);

    let local_config = require('../' + type + "/" + feature + '/conf.json');

    if (config.debug === true) console.log(local_config);
    ...
    if (config.debug === true) console.log(conf_merged[type][index]);
};
